...
def save_images2(images, size, image_path):
    return scipy.misc.imsave(image_path, scipy.misc.imresize(images, size, 'bilinear'))
...
def visualize(sess, dcgan, config, option, gan_set=0):
    ...
    if option == 0:
        ...
        for j in xrange(int(config.sample_size / config.batch_size)):
            ...
            #make_gif(image, './gif/gan_%s_%s.gif' % (gan_set, j), duration=20)
            if not os.path.isdir(config.output_path):
                os.mkdir(config.output_path)
            for i in xrange(config.batch_size):
                print(" [*] %s" % str(64 * j + i))
                save_images2(image[i], [224, 224, 3, 3, 3], "./%s/gan_%d_%d.jpg" % (config.output_path, gan_set, ind))
                ind = ind + 1
